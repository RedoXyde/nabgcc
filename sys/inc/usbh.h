...
/****************************************************************************/
#define usbh_set_bulk_urb(urb, dev, ep, buf, size, callbak) \
	do{	\
		PURB purb = (urb); \
		purb->buffer	  = buf; \
		purb->length	  = size ; \
		...
		purb->dev		  = dev; \
		purb->ed		  = dev->pipe[ep]; \
		purb->callback	  = callbak; \
	}while(0)

#define usbh_set_interrupt_urb(urb, dev, ep, buf, size, callbak) \
	do{	\
		PURB purb = (urb); \
		purb->buffer	  = buf; \
		purb->length	  = size ; \
...
void *usbh_malloc(unsigned long);
void *usbh_malloc_ex(unsigned long);
int usbh_free(void *);
...
int usbh_bulk_transfer(PDEVINFO, uchar, void *, ulong, uchar);
...
int usbh_set_configuration(PDEVINFO, ushort);
  ...
  #define USBH_MALLOC_INIT() \
  	hcd_malloc_init(USB_BUFFER_ADDRESS, USB_BUFFER_SIZE, 1, EXTRAM)
  #define malloc(a) usbh_malloc_ex(a)
  #define free(a)	usbh_free(a)
#else
  #define USBH_MALLOC_INIT() do{;}while(0)
#endif
